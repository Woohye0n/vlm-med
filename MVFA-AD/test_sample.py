...
def main():
    ...
    for idx, obj in enumerate(classes):
        ...
        result = test(args, obj, model, test_loader, text_features, seg_mem_features, det_mem_features)
...
def test(args, obj, model, test_loader, text_features, seg_mem_features, det_mem_features):
    ...
    for (image, y, mask) in tqdm(test_loader):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            print(anomaly_map_few_shot.shape)
            print(anomaly_map.shape)
            anomaly_map = anomaly_map.reshape(1, 1, 17, 17)  # [B, C, H, W]
            anomaly_map_upsampled = F.interpolate(anomaly_map, size=(240, 240), mode='bilinear', align_corners=False)
            anomaly_map_np = anomaly_map_upsampled.squeeze().cpu().numpy()  # shape: (240, 240)

            # 2. anomaly_map_few_shot: squeeze to (240, 240)
            anomaly_map_few_shot = anomaly_map_few_shot.squeeze()  # (240, 240)

            # 3. 평균합
            temp = (anomaly_map_np + anomaly_map_few_shot) / 2
            ...
            # 5. 저장
            img = Image.fromarray(temp, mode='L')
            img.save("temp.png")
