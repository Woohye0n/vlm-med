...
args = {
    ...
    # 'anomalygpt_ckpt_path': '/home/aidas_2/.cache/huggingface/hub/models--FantasticGNU--AnomalyGPT/snapshots/3fcef88ce8977c4065f4a4372c14519568a0cc92/train_supervised/pytorch_model.pt',
    'anomalygpt_ckpt_path': '/home/aidas_2/.cache/huggingface/hub/models--FantasticGNU--AnomalyGPT/snapshots/3fcef88ce8977c4065f4a4372c14519568a0cc92/train_mvtec/pytorch_model.pt',
...
for c_name in CLASS_NAMES:
    # normal_img_paths = ["../data/bmad/"+c_name+"/train/good/"+str(command_args.round * 4).zfill(3)+".png", "../data/bmad/"+c_name+"/train/good/"+str(command_args.round * 4 + 1).zfill(3)+".png",
    #                     "../data/bmad/"+c_name+"/train/good/"+str(command_args.round * 4 + 2).zfill(3)+".png", "../data/bmad/"+c_name+"/train/good/"+str(command_args.round * 4 + 3).zfill(3)+".png"]
    normal_img_paths = os.listdir("../data/bmad/"+c_name+"/train/good/")
    
    normal_img_paths = ["../data/bmad/"+c_name+"/train/good/" + normal_img_paths[i] for i in range(0, command_args.k_shot * command_args.round * 4, command_args.round * 4)]
    right = 0
    wrong = 0
    p_pred = []
    p_label = []
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            ...
            if "test" in file_path and 'png' in file and c_name in file_path:
                if FEW_SHOT:
                    resp, anomaly_map = predict(describles[c_name] + ' ' + input, file_path, normal_img_paths, 8, 0.1, 1.0, [], [])
                else:
                    resp, anomaly_map = predict(describles[c_name] + ' ' + input, file_path, [], 8, 0.1, 1.0, [], [])
