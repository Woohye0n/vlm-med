...
def preprocess_caption(
    ...
    conversations = []
    for i, source in enumerate(sources):
        # Skip non-human first messages if any
        if roles[source[0]["from"]] != conv.roles[0]:
            print(f"Skipping the first one if it is not from human: {i}")
            source = source[1:]
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "caption":
        return preprocess_caption(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
